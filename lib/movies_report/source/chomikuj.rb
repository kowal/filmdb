...
module MoviesReport
  ...
  module Source
      ...
      def predefined_page_info(page_name)
        {
          ...
          file_list: {
            ...
            fields: {
              title: ->(el) do
                sanitize_title(el.css('.filename').first.content.strip)
              end,
              size:  ->(el) do
                el.css('.fileinfo li:nth-last-child(2)').first.content
              end
            }
          }
        }[page_name]
    ...
    class ChomikujSanitizer
      ...
      TO_REMOVE = %w{
        .BRRiP MX PL.DVDRiP.XviD.AC3-PBWT PL.DVDRiP.XViD-J25 DVDRip.XviD-LEKTOR HDRip.XviD-LEKTOR PL.PDTV.XViD-Zet
